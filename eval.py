...
def load_model(args, cfg):
    ...
    trainer = trainer_lib.Trainer(cfg, args, device='cuda:0')
...
def inference(model, depth, mask, bb, K, shape_handler, class_id, num_segs=256):
    def return_with_zero():
        return 1,np.eye(3),np.array([0,0,0]),np.eye(4), np.array([1,1,1]), np.zeros((128), None)
    ...
    s, R, t, T = estimateSimilarityTransform(points.copy(), attrs[pred_label,1:], verbose=True)
    T[:3,:3] = T[:3,:3].T
    ...
    return s, R, t, Rt, bbox, shapecode, points/1000.0

def infer_from_points(model, points, shape_handler, class_id, num_segs=256):
    def return_with_zero():
        return 1,np.eye(3),np.array([0,0,0]),np.eye(4), np.array([1,1,1]), np.zeros((128)), None
